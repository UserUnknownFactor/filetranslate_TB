...
init: function () {
    ...
    try {
        ...
        this.log("Could not retrieve saved language preference, using defaults");
    }
        ...
        try {
            ...
            if (absolutePath.includes(".asar")) {
                ...
                if (fs.existsSync(asarPath)) {
                    // Use original-fs for asar access
                    const originalFs = window.require("original-fs");
                    return originalFs.existsSync(asarPath + relativePath);
                }
            }
...
tryLoadTranslations: function (scenario) {
    ...
    if (this.existsSync(translationPathStrings) &&
        ...
        if (!dataString) {
            this.log(`Error loading string translations for ${scenario}`);
            return;
        }
    ...
    if (this.existsSync(translationPathAttributes) &&
        ...
        if (!dataAttr) {
            this.log(`Error loading attribute translations for ${scenario}`);
            return;
        }
    ...
    for (let i = 0; i < array_s.length; i++) {
        ...
        if (tobj.name === "iscript") {
            ...
            if (tobj.pm && tobj.pm.name) {
                let chara_name = tobj.pm.name;
                if (this.data.characters[chara_name]) {
                    if (this.kag.stat.charas[chara_name])
                        this.kag.stat.charas[chara_name].jname = this.kag.stat.charas[chara_name];
                    else
...
loadCharacterTranslations: function () {
    ...
    if (this.existsSync(charTranslationPath) &&
        ...
        if (!dataAttr) {
            this.log(`Error loading character translations for ${scenario}`);
            return;
        }
