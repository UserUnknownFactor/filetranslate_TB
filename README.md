...
A text and image translation plugin for TyranoBuilder/TyranoScript that enables multi-language support for your visual novel projects.
...
1. Copy the plugin code to `app/tyrano/plugins/kag/kag.translation.js` in your TyranoBuilder project;
2. Translate `app/tyrano/langs.js`;
2. Register the plugin in `app/index.html` as `<script type="text/javascript" src="tyrano/plugins/kag/kag.translation.js" ></script>` after all other plugins.
...
Without `_fr` part it'd be considered default/unset translation (English). 
If no translation data found default game strings and images are used.
...
Special characters:
- `¶` for escaping (`\n`, `¶`, `→` characters)
- `→` as separator between original and translated text
...
```
https://yourgame.com/index.html?lang=en
```
